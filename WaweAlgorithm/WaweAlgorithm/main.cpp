...
void CHANGE_B(int* x, int* y, int* xA, int* yA)
{
	...
	while (check)
	{
		std::cout << "Введите координаты B (1 - 20):" << std::endl;
		std::cout << "X: "; std::cin >> *x;
		std::cout << "Y: "; std::cin >> *y;
		...
		if ((*xA == *x) && (*yA == *y))
		{
			std::cout << "ОШИБКА: Точка B совпадает с точкой A, попробуйте снова: " << std::endl;
			continue;
		}
		...
		if (FIELD[*x][*y] == 888)
		{
			std::cout << "ОШИБКА: Точка В попала на препятствие, повторите снова: " << std::endl;
		}
		else check = false;
	}
